package wasambo.erick.job.interview.adapter;
...
import wasambo.erick.job.interview.model.WeatherViewHolder;
    ...
    public void onBindViewHolder(WeatherViewHolder customViewHolder, int i) {
        ...
        if (sp.getString("speedUnit", "m/s").equals("bft")) {
            customViewHolder.itemyWind.setText(context.getString(R.string.wind) + ": " +
                    UnitConvertor.getBeaufortName((int) wind, context) + " " + BookMarkedCityActivity.getWindDirectionString(sp, context, weatherItem));
        } else {
            customViewHolder.itemyWind.setText(context.getString(R.string.wind) + ": " + new DecimalFormat("0.0").format(wind) + " " +
                    BookMarkedCityActivity.localize(sp, context, "speedUnit", "m/s")
                    + " " + BookMarkedCityActivity.getWindDirectionString(sp, context, weatherItem));
        }
        customViewHolder.itemPressure.setText(context.getString(R.string.pressure) + ": " + new DecimalFormat("0.0").format(pressure) + " " +
                BookMarkedCityActivity.localize(sp, context, "pressureUnit", "hPa"));
